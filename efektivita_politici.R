...
efektivitaStarostu <-
    ggplot(efCerp,
           aes(
               x = reorder(misto30, efektivitacerpani6),
               y = efektivitacerpani6,
               fill = factor(star30)
           )) +
...
ggiraph({
    print(efektivitaStarostu)
})

####
podil_cerpani_zdroj <-
    read.csv(
        "data/podil_kapital.csv",
        header = TRUE,
        encoding = "UTF-8",
        sep = ","
    )
...
podil_cerpani <-
    ggplot(podil_cerpani_zdroj,
           aes(
               x = reorder(misto30, podil_cerpano),
               y = podil_cerpano,
               fill = factor(star30)
           )) +
    geom_bar(stat = "identity") + scale_fill_manual(values = barva) +
...
####
efCerp <-
    read.csv("data/rozpocty_polit.csv",
             header = TRUE,
             encoding = "UTF-8")
...
ggiraph({
    print(boxplot_cerpani_kapitalu)
})
